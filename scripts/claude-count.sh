...
# bell + waiting. "waiting" = sitting on a permission/decision prompt; "thinking" =
...
# also tags each pane with @claude_waiting for pane-border-format styling.
set -euo pipefail
...
while IFS=$'\t' read -r cmd pid; do
  ...
  cur=$(tmux show -p -t "$pid" -v @claude_waiting 2>/dev/null || true)
  if [ "$cur" != "$state" ]; then
    tmux set -p -t "$pid" @claude_waiting "$state" 2>/dev/null || true
  fi
...
# waiting — then the thin-chevron segment separator (U+E0B3), matching clock/calendar.
sep=$'\xee\x82\xb3'
printf '#[fg=yellow]󰚩 #[fg=white]%d #[fg=yellow]󰒓 #[fg=white]%d #[fg=yellow]󰂚 #[fg=white]%d %s ' \
  "$total" "$thinking" "$waiting" "$sep"
exit 0
